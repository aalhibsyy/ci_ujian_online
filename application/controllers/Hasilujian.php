...
class HasilUjian extends CI_Controller {
	...
	public function data()
	{
		...
		if( $this->ion_auth->in_group('pengajar') ) {
			$nip_pengajar = $this->user->username;
		}

		$this->output_json($this->ujian->getHasilUjian($nip_pengajar), false);
	}
	...
	public function cetak($id)
	{
		...
		$ssw 	= $this->ujian->getIdSiswa($this->user->username);
		$hasil 	= $this->ujian->HslUjian($id, $ssw->id_siswa)->row();
		$ujian 	= $this->ujian->getUjianById($id);
