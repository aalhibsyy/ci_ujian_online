...
class Ujian extends CI_Controller {
    ...
    public function master()
	{
        ...
        $data = [
			...
			'judul'	=> 'Ujian',
			'subjudul'=> 'Data Ujian',
			'pengajar' => $this->ujian->getIdPengajar($user->username),
		];
	...
	public function validasi()
	{
		...
		$user 	= $this->ion_auth->user()->row();
		$pengajar 	= $this->ujian->getIdPengajar($user->username);
		$jml 	= $this->ujian->getJumlahSoal($pengajar->id_pengajar)->jml_soal;
	...
	public function save()
	{
		...
		$diklat_id 		= $this->input->post('diklat_id', true);
		...
		if( $this->form_validation->run() === FALSE ){
			...
			if($method === 'add'){
				$input['pengajar_id']	= $pengajar_id;
				$input['diklat_id'] = $diklat_id;
				$input['token']		= $token;
	...
	public function list_json()
	{
		...
		$list = $this->ujian->getListUjian($this->ssw->id_siswa, $this->ssw->diklat_id);
	...
	public function list()
	{
		...
		$data = [
			...
			'judul'		=> 'Ujian',
			'subjudul'	=> 'List Ujian',
			'ssw' 		=> $this->ujian->getIdSiswa($user->username),
		];
		$this->load->view('_templates/dashboard/_header.php', $data);
	...
	public function index()
	{
		...
		$ssw		= $this->ssw;
		$h_ujian 	= $this->ujian->HslUjian($id, $ssw->id_siswa);
