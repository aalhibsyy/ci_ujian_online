...
class Soal extends CI_Controller {
	...
	public function __construct(){
		...
		if (!$this->ion_auth->logged_in()){
			redirect('auth');
		}else if ( !$this->ion_auth->is_admin() && !$this->ion_auth->in_group('pengajar') ){
			show_error('Hanya Administrator dan pengajar yang diberi hak untuk mengakses halaman ini, <a href="'.base_url('dashboard').'">Kembali ke menu awal</a>', 403, 'Akses Terlarang');
		}
    ...
    public function index()
	{
        ...
        if($this->ion_auth->is_admin()){
            //Jika admin maka tampilkan semua diklat
            $data['diklat'] = $this->master->getAllDiklat();
        }else{
            //Jika bukan maka diklat dipilih otomatis sesuai diklat pengajar
            $data['diklat'] = $this->soal->getDiklatPengajar($user->username);
        }
    ...
    public function add()
	{
        ...
        if($this->ion_auth->is_admin()){
            ...
            //Jika bukan maka diklat dipilih otomatis sesuai diklat pengajar
            $data['pengajar'] = $this->soal->getDiklatPengajar($user->username);
        }
	...
	public function data($id=null, $pengajar=null)
	{
		$this->output_json($this->soal->getDataSoal($id, $pengajar), false);
    }
    ...
    public function save()
    {
        ...
        if($this->form_validation->run() === FALSE){
            ...
            if($this->ion_auth->is_admin()){
                ...
                $data['diklat_id'] = end($pecah);
            }else{
                $data['pengajar_id'] = $this->input->post('pengajar_id', true);
                $data['diklat_id'] = $this->input->post('diklat_id', true);
            }
