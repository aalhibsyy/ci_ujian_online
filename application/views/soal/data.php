		...
		<div class="row">
			...
			<div class="form-group col-sm-4 text-center">
				<?php if ( $this->ion_auth->is_admin() ) : ?>
					<select id="diklat_filter" class="form-control select2" style="width:100% !important">
						<option value="all">Semua diklat</option>
						<?php foreach ($diklat as $m) :?>
							<option value="<?=$m->id_diklat?>"><?=$m->nama_diklat?></option>
						<?php endforeach; ?>
					</select>
				<?php endif; ?>
				<?php if ( $this->ion_auth->in_group('pengajar') ) : ?>				
					<input id="diklat_id" value="<?=$diklat->nama_diklat;?>" type="text" readonly="readonly" class="form-control">
				<?php endif; ?>
			</div>
			<div class="col-sm-4">
...
$(document).ready(function(){
	$('#diklat_filter').on('change', function(){
		let id_diklat = $(this).val();
		let src = '<?=base_url()?>soal/data';
		let url;

		if(id_diklat !== 'all'){
			let src2 = src + '/' + id_diklat;
...
<script type="text/javascript">
$(document).ready(function(){
	let id_diklat = '<?=$diklat->diklat_id?>';
	let id_pengajar = '<?=$diklat->id_pengajar?>';
	let src = '<?=base_url()?>soal/data';
	let url = src + '/' + id_diklat + '/' + id_pengajar;
