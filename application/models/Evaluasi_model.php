...
class Evaluasi_model extends CI_Model {
    ...
    public function get_kdeval(){
        $q = $this->db->query("SELECT MAX(RIGHT(kd_evaluasi,4)) AS kd_max FROM evaluasi");
        $kd = "";
        if ($q->num_rows() > 0) {
    ...
    public function countPenilaian($id)
    {
        $this->db->where('diklat_id', $id);
        $num_rows = $this->db->count_all_results('penilaian');
        return $num_rows;
    }

    public function getListEvaluasi($id, $diklat)
    {
        
        $this->datatables->select("*,a.id_materi, (SELECT COUNT(kd_evaluasi) FROM evaluasi h WHERE h.siswa_id = {$id} AND h.materi_id = a.id_materi) AS ada");
        
        $this->datatables->from('materi a');
        $this->datatables->join('diklat b', 'a.diklat_id = b.id_diklat');
        $this->datatables->join('pengajar c', 'c.diklat_id=b.id_diklat');
        //$this->datatables->join('evaluasi d', 'd.materi_id=a.id_materi');
        $this->datatables->where('a.diklat_id', $diklat);
        return $this->datatables->generate();
    }
    
    public function getMateriEval($id)
    {
        $this->db->select('*');
        $this->db->from('materi');
    ...
    public function getPenilaianEval($id)
    {
        $this->db->select('*');
        $this->db->from('penilaian a');
        $this->db->join('materi b','b.id_materi=a.materi_id');
