function loadJurusan(id) {
    ...
    $.getJSON(base_url+'jurusandiklat/getJurusanId/' + id, function (data) {
        console.log(data);
        let opsi;
...
$(document).ready(function () {
    $('[name="diklat_id"]').on('change', function () {
        loadJurusan($(this).val());
    });

    $('form#jurusandiklat select').on('change', function () {
        $(this).closest('.form-group').removeClass('has-error');
        $(this).nextAll('.help-block').eq(0).text('');
    });

    $('form#jurusandiklat').on('submit', function (e) {
        e.preventDefault();
            ...
            success: function (data) {
                ...
                if (data.status) {
                    Swal({
                        ...
                        if (result.value) {
                            window.location.href = base_url+'jurusandiklat';
                        }
                    });
                } else {
                    if (data.errors) {
